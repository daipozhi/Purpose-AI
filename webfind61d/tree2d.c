...
int t4_new_node(void)
{
  ...
  if ((t4_buff_pp<TREE2_SIZE_D)&&(t4_node_mark[t4_buff_pp]<0))
  {
...
int t4_insert_node(int pn1,int pn2,int pn3,int pn4,int pn5,int pn6)
{
  ...
  else
  {
    if (t4_find_pp<0)
    {
      ...
      if (j<0)
      {
        MessageBox(0,"In tree2d,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
  ...
  while (t4_list_pp>0)
  {
    ...
    if (t4_list_stack_type[j]==1)
    {
      ...
      if (t4_node_pp[k][1]>=0)
      {
        ...
        if (t4_list_pp>=LIST_SIZE_D)
        {
          MessageBox(0,"In tree2d,error in after_list(),LIST_SIZE_D too small.","message",MB_OK);
          continue;
        }
      }
...
int t4_out_list(char *pstr,long long int pn1,int pp)
{
  int i,j;
/*
  if ((int)strlen(pstr)>=300) return(0);
  
  strcpy(t4_out_buff[t4_out_pp],pstr);
  t4_out_buff2[t4_out_pp]=pn1;


  for (i=0;i<5;i++)
	for (j=0;j<3;j++)
		t4_out_buff3[t4_out_pp][i][j]=t4_node_val3[pp][i][j];
