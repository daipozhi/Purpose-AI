...
//		MessageBox(0,m101_str3,"into pop-r2",MB_OK);
			...
			for (i=6;i>=2;i--)     // bigger ones at first  // add grammer in grammer courseware
			{
				...
				if (nn==0)
				{
					...
					m101_p_ff1=((m101_p_va2+                ai_number[i3/2])*1000000)/(j7+i10);
					m101_p_ff2=((m101_p_rp2+t1_node_val2[t1_find_pp]*(i3/2))*1000000)/(j7+i10);
					m101_p_ff3=(m101_p_val*1000000)/(j7+i10);
					m101_p_ff4=(m101_p_rpt*1000000)/(j7+i10);
			...
			for (i=5;i>=2;i--)     // bigger ones at first  // add grammer in grammer base 
			{
				...
				for (m=0;m<i4;m++) // for every mark
				{
					...
					if (nn==0)
					{
						...
						m101_p_ff1=(m101_p_va2*1000000)/(j7+i10);
						m101_p_ff2=(m101_p_rp2*1000000)/(j7+i10);
						m101_p_ff3=((m101_p_val+                ai_number[i3/2])*1000000)/(j7+i10);
						m101_p_ff4=((m101_p_rpt+t3_node_val2[t3_find_pp]*(i3/2))*1000000)/(j7+i10);
...
//					MessageBox(0,m101_str3,"into pop-r1",MB_OK);
			...
			m101_p_ff2=(m101_p_rp2*1000000)/(j7+i10);
			m101_p_ff3=((m101_p_val+ai_number[i3/2])*1000000)/(j7+i10);
			m101_p_ff4=((m101_p_rpt+       1*(i3/2))*1000000)/(j7+i10);
...
/*
	...
	while (i2<BTREE8_SIZE)
	{
		...
		if ((t8_node_mark[n1]==0)&&(t8_node_v_mrk[n1]==2))
		{
			...
			if (t8_node_v_va2[n1]>spl2_out_val) bigger=1;
			else
			{
				if (t8_node_v_va2[n1]==spl2_out_val)
				{
					if (t8_node_v_rp2[n1]>spl2_out_va2) bigger=1;
					else
					{
						if (t8_node_v_rp2[n1]==spl2_out_va2)
						{
							if (t8_node_v_val[n1]>spl2_out_va3) bigger=1;
							else
							{
								if (t8_node_v_val[n1]==spl2_out_va3)
								{
									if (t8_node_v_rpt[n1]>spl2_out_va4) bigger=1;
								}
							}
			...
			if (bigger==1)
			{
				...
				for (i6=0;i6<t8_node_v_seg[n1];i6++)
				{
					...
					spl2_out_mrk[i6]=t8_node_v_mr2[n1][i6]; //grammer mark ,in cw or in base
