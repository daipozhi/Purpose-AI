...
#define TREE2_SIZE_B 5000000
#define LIST_SIZE_B  500000

    char  t2_node_mark[TREE2_SIZE_B];
    int   t2_node_val[TREE2_SIZE_B][5];
    int   t2_node_val2[TREE2_SIZE_B];
    ...
    //int   t2_out_buff[TREE2_SIZE_B][5];
    ...
    int   t2_search_node(int pn1,int pn2,int pn3,int pn4,int pn5);
    int   t2_insert_node(int pn1,int pn2,int pn3,int pn4,int pn5);
...
int t2_new_node(void)
{
  ...
  while (1)
  {
    if (t2_node_mark[t2_buff_pp]<0)
    {
      t2_node_mark[t2_buff_pp]=0;
      i=t2_buff_pp;
      t2_buff_pp++;
      if (t2_buff_pp>=TREE2_SIZE_B) t2_buff_pp=0;
      break;
    }
    else
    {
...
int t2_search_node(int pn1,int pn2,int pn3,int pn4,int pn5)
{
  ...
  while (1)
  {
    ...
    if (t2_istrcmp(i,pn1,pn2,pn3,pn4,pn5)==0)
    {
      t2_find_pp=i;
      return(0);
    }
...
int t2_insert_node(int pn1,int pn2,int pn3,int pn4,int pn5)
{
  ...
  i=t2_search_node(pn1,pn2,pn3,pn4,pn5);
...
int t2_istrcmp(int i,int pn1,int pn2,int pn3,int pn4,int pn5)
{
	int j;

	if (t2_node_val[i][0]>pn1) return(1);
	else
	{
		if (t2_node_val[i][0]<pn1) return(-1);
		else
	...
	if (t2_node_val[i][1]>pn2) return(1);
	else
	{
		if (t2_node_val[i][1]<pn2) return(-1);
		else
	...
	if (t2_node_val[i][2]>pn3) return(1);
	else
	{
		if (t2_node_val[i][2]<pn3) return(-1);
		else
	...
	if (t2_node_val[i][3]>pn4) return(1);
	else
	{
		if (t2_node_val[i][3]<pn4) return(-1);
		else
	...
	if (t2_node_val[i][4]>pn5) return(1);
	else
	{
		if (t2_node_val[i][4]<pn5) return(-1);
		else
...
int t2_out_list(char *pstr,long long int pn1,int pp)
{
  int i,j;
/*
...
int t2_save_list(char *fn)
{
  ...
  {
    fputs(t2_out_buff[i],fp);
    //fputs(",",fp);

    //sprintf(m05_str1,"%lld",t2_out_buff2[i]);
    //fputs(m05_str1,fp);
