...
int main(int argc,char **argv)
{
	...
	wd5_load();  // word database  // 装入词库

	wd6_load();  // word courseware 2  // 装入词课件

	cww1_load(); // word courseware 1 //装入1号词课件，用于判断一个字符串是不是数字

	wd7_sub_load(); // sub word courseware   // 装入子词课件

	//grm15_load();  //grammar database //remove grm15;  //装入语法库，已经删除

	grm16_load();  //grammar courseware  //装入语法课件

        load_cb2();  // load punctuation 2  // 装入2号标点符号库
...
int mproc(char *strpath)
{
	...
	i=f1_init_ext();        //取文件名函数初始化,并取出一个文件名，准备处理
	if (i==1) return(0);    
...
int f1_get_one(void)   //从文件名库中取出一个文件名
{
  FILE *fp1;
  int   i,j,endf;
  char  str1[300];
...
int f1_init_ext(void)  //取文件名函数的初始化,并取出一个文件名
{
	int i;
...
int frame_loop1(void)
{
	...
	s_fp1=fopen(m101_str5,"r");      //打开输入文件
	if (s_fp1==NULL)
	{
...
//第5，6，7，8个程序 ：btree5.c btree6.c btree7.c btree8.c
