...
#define LIST_SIZE_H  30000

    char  t8_node_mark[TREE2_SIZE_H];
    float t8_node_val[TREE2_SIZE_H][4];
    int   t8_node_val2[TREE2_SIZE_H];
    int   t8_node_val3[TREE2_SIZE_H];

    int   t8_node_pp[TREE2_SIZE_H][3];
    int   t8_root_pp;
    int   t8_buff_pp;
    ...
    int   t8_list_stack[LIST_SIZE_H];
    char  t8_list_stack_type[LIST_SIZE_H];
    int   t8_list_pp;

    float t8_out_buff[TREE2_SIZE_H][4];
    int   t8_out_buff2[TREE2_SIZE_H];
    ...
    int   t8_search_node(float pn1,float pn2,float pn3,float pn4,int pn5);
    int   t8_insert_node(float pn1,float pn2,float pn3,float pn4,int pn5);
    int   t8_istrcmp(int i,float pn1,float pn2,float pn3,float pn4,int pn5);
    int   t8_dsp_tree2(void);
...
int t8_new_node(void)
{
  ...
  while (1)
  {
    if (t8_node_mark[t8_buff_pp]<0)
    {
      t8_node_mark[t8_buff_pp]=0;
      i=t8_buff_pp;
      t8_buff_pp++;
      if (t8_buff_pp>=TREE2_SIZE_H) t8_buff_pp=0;
      break;
    }
    else
    {
...
int t8_search_node(float pn1,float pn2,float pn3,float pn4,int pn5)
{
  ...
  while (1)
  {
    ...
    if (t8_istrcmp(i,pn1,pn2,pn3,pn4,pn5)==0)
    {
      t8_find_pp=i;
      return(0);
    }
...
int t8_istrcmp(int i,float pn1,float pn2,float pn3,float pn4,int pn5)
{
	...
	if (t8_node_val2[i]>pn5) return(1);
	else
	{
		if (t8_node_val2[i]<pn5) return(-1);
		else
...
int t8_out_list(int pp)
{
  ...
  t8_out_buff[t8_out_pp][0]=t8_node_val[pp][0];
  t8_out_buff[t8_out_pp][1]=t8_node_val[pp][1];
  t8_out_buff[t8_out_pp][2]=t8_node_val[pp][2];
  t8_out_buff[t8_out_pp][3]=t8_node_val[pp][3];

  t8_out_buff2[t8_out_pp]=t8_node_val2[pp];
  t8_out_buff3[t8_out_pp]=t8_node_val3[pp];

  //t8_out_buff3_pp[t8_out_pp]=t8_node_val3_pp[pp];
...
int t8_save_list(char *fn)
{
  ...
  for (i=0;i<t8_out_pp;i++)
  {
    fputs(t8_out_buff[i],fp);
    //fputs(",",fp);

    //sprintf(m05_str1,"%lld",t8_out_buff2[i]);
    //fputs(m05_str1,fp);
