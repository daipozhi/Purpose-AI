...
//extern int MessageBoxNow(int ,char *,char *,int );

#define TREE_SIZE 10000000
#define LIST_SIZE  1000000

    char  t5_node_mark[TREE_SIZE];
    char  t5_node_val[TREE_SIZE][55];

    int   t5_node_val2[TREE_SIZE];

    int   t5_node_ptr[TREE_SIZE][3];
    int   t5_root_ptr;
    int   t5_buff_ptr;
    ...
    int   t5_list_stack[LIST_SIZE];
    char  t5_list_stack_type[LIST_SIZE];
    int   t5_list_ptr;
    ...
    int   t5_search_node(char *pstr,int);
    int   t5_insert_node(char *pstr,int);
...
int /*tree2::*/t5_new_node(void)
{
  ...
  if ((t5_buff_ptr<TREE_SIZE)&&(t5_node_mark[t5_buff_ptr]<0))
  {
...
int /*tree2::*/t5_insert_node(char *pstr,int pstr_size)
{
  ...
  i=t5_search_node(pstr,pstr_size);
...
/*
  ...
  if (fp==NULL)
  {
    MessageBoxNow(0,m05_str2,"Error at open file",MB_OK);
    return(1);
  }
  ...
  for (i=0;i<t5_out_ptr;i++)
  {
    ...
    for (j=0;j<t5_out_buff3_ptr[i];j++)
	for (k=0;k<3;k++)
	{
		l=t5_out_buff3[i][j][k];
		if (l<0) l=0;
