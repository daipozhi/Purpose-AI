...
#define TREE2_SIZE_F 3000000
#define LIST_SIZE_F  300000

    char  t6_node_mark[TREE2_SIZE_F];
    int   t6_node_val[TREE2_SIZE_F][4/*5*/];
    int   t6_node_val2[TREE2_SIZE_F];

    int   t6_node_ptr[TREE2_SIZE_F][3];
    int   t6_root_ptr;
    int   t6_buff_ptr;
    ...
    //int   t6_out_buff[TREE2_SIZE_F][5];
    ...
    int   t6_out_list(char *pstr,long long int ,int);
...
int t6_new_node(void)
{
  ...
  if ((t6_buff_ptr<TREE2_SIZE_F)&&(t6_node_mark[t6_buff_ptr]<0))
  {
...
int t6_insert_node(int pn1,int pn2,int pn3,int pn4/*,int pn5*/)
{
  ...
  else
  {
    if (t6_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBox(0,"In tree2f,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
  ...
  while (t6_list_ptr>0)
  {
    ...
    if (t6_list_stack_type[j]==1)
    {
      ...
      if (t6_node_ptr[k][1]>=0)
      {
        ...
        {
          MessageBox(0,"In tree2f,error in after_list(),LIST_SIZE_F too small.","message",MB_OK);
          continue;
        }
      }
...
int t6_out_list(char *pstr,long long int pn1,int ptr)
{
  int i,j;
/*
