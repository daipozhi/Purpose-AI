...
int t6_new_node(void)
{
  ...
  if ((t6_buff_pp<TREE2_SIZE_F)&&(t6_node_mark[t6_buff_pp]<0))
  {
    t6_node_mark[t6_buff_pp]=0;
    i=t6_buff_pp;
    t6_buff_pp++;
  }
...
int t6_out_list(char *pstr,long long int pn1,int pp)
{
  int i,j;
/*
  if ((int)strlen(pstr)>=300) return(0);
  
  strcpy(t6_out_buff[t6_out_pp],pstr);
  t6_out_buff2[t6_out_pp]=pn1;


  for (i=0;i<5;i++)
	for (j=0;j<3;j++)
		t6_out_buff3[t6_out_pp][i][j]=t6_node_val3[pp][i][j];

  t6_out_buff3_pp[t6_out_pp]=t6_node_val3_pp[pp];
