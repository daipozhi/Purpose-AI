...
int t5_old_node(int ptr)
{
   ...
   {
     MessageBoxNow(0,"In btree5,error at tn_old_node()","message",MB_OK);
     return(-1);
   }
   else
   {
...
int t5_insert_node(long long int pn1,long long int pn2,long long int pn3,long long int pn4)
{
  ...
  else
  {
    if (t5_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBoxNow(0,"In btree5,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
  ...
  while (t5_list_ptr>0)
  {
    ...
    if (t5_list_stack_type[j]==1)
    {
      ...
      if (t5_node_ptr[k][1]>=0)
      {
        ...
        {
          MessageBoxNow(0,"In btree5,error in after_list(),BTREE5_LSIZE too small.","message",MB_OK);
          continue;
        }
      }
