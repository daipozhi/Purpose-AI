...
int main(void)
{
	MessageBox(0,"load words-cw2-000.txt , word database, words courseware, grammar database, grammar courseware, write to grm-base1-000.txt","message",MB_OK);
		...
		if (m101_sent[0]!=0)
		{
			...
			else
			{
				...
				for (i=0;i<spl2_out_seg;i++)
				{
					...
					{
						k =spl2_out_nns[i][j];
						k2=spl1_out_nns[spl2_pp2][m+j];

						if (k==(-2)) {  fputs("$*((",m_fp1);  fputs(                wd5_buf[k2],m_fp1);  fputs("))",m_fp1); }
						if (k==(-3)) {  fputs("$n((",m_fp1);  fputs(spl1_out_str[spl2_pp2][m+j],m_fp1);  fputs("))",m_fp1); }
