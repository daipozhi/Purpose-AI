...
int t3_insert_node(int pn1,int pn2,int pn3,int pn4/*,int pn5*/)
{
  ...
  else
  {
    if (t3_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBoxNow(0,"In tree2c,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
  ...
  while (t3_list_ptr>0)
  {
    ...
    if (t3_list_stack_type[j]==1)
    {
      ...
      if (t3_node_ptr[k][1]>=0)
      {
        ...
        {
          MessageBoxNow(0,"In tree2c,error in after_list(),LIST_SIZE_C too small.","message",MB_OK);
          continue;
        }
      }
