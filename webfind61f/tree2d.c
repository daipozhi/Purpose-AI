...
#define LIST_SIZE_D  5000
    ...
    int   t4_node_val[TREE2_SIZE_D][6];
    //int   t4_node_val2[TREE2_SIZE_D];

    int   t4_node_ptr[TREE2_SIZE_D][3];
    int   t4_root_ptr;
    int   t4_buff_ptr;
    ...
    //int   t4_out_buff[TREE2_SIZE_D][6];
    ...
    int   t4_out_list(char *pstr,long long int ,int);
...
int t4_new_node(void)
{
  ...
  if ((t4_buff_ptr<TREE2_SIZE_D)&&(t4_node_mark[t4_buff_ptr]<0))
  {
    t4_node_mark[t4_buff_ptr]=0;
    i=t4_buff_ptr;
    t4_buff_ptr++;
  }
...
int t4_search_node(int pn1,int pn2,int pn3,int pn4,int pn5,int pn6)
{
  ...
  while (1)
  {
    ...
    if (t4_istrcmp(i,pn1,pn2,pn3,pn4,pn5,pn6)==0)
    {
      t4_find_ptr=i;
      return(0);
    }
...
int t4_insert_node(int pn1,int pn2,int pn3,int pn4,int pn5,int pn6)
{
  ...
  i=t4_search_node(pn1,pn2,pn3,pn4,pn5,pn6);
  ...
  else
  {
    if (t4_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBoxNow(0,"In tree2d,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
...
int t4_istrcmp(int i,int pn1,int pn2,int pn3,int pn4,int pn5,int pn6)
{
	...
	if (t4_node_val[i][4]>pn5) return(1);
	else
	{
		if (t4_node_val[i][4]<pn5) return(-1);
		else
	...
	if (t4_node_val[i][5]>pn6) return(1);
	else
	{
		if (t4_node_val[i][5]<pn6) return(-1);
		else
  ...
  while (t4_list_ptr>0)
  {
    ...
    if (t4_list_stack_type[j]==1)
    {
      ...
      if (t4_node_ptr[k][1]>=0)
      {
        ...
        {
          MessageBoxNow(0,"In tree2d,error in after_list(),LIST_SIZE_D too small.","message",MB_OK);
          continue;
        }
      }
    ...
    else
    {
      ...
      //t4_out_list(t4_node_val[k],t4_node_val2[k],k);
...
int t4_out_list(char *pstr,long long int pn1,int ptr)
{
  int i,j;
/*
  if ((int)strlen(pstr)>=300) return(0);
  
  strcpy(t4_out_buff[t4_out_ptr],pstr);
  t4_out_buff2[t4_out_ptr]=pn1;


  for (i=0;i<5;i++)
	for (j=0;j<3;j++)
		t4_out_buff3[t4_out_ptr][i][j]=t4_node_val3[ptr][i][j];

  t4_out_buff3_ptr[t4_out_ptr]=t4_node_val3_ptr[ptr];
