...
int /*tree2::*/t5_new_node(void)
{
  ...
  if ((t5_buff_ptr<TREE_SIZE_E)&&(t5_node_mark[t5_buff_ptr]<0))
  {
...
int /*tree2::*/t5_insert_node(char *pstr,int pstr_size)
{
  ...
  else
  {
    if (t5_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBox(0,"In tree5,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
