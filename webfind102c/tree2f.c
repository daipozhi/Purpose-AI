...
#define TREE_SIZE_F 15000000
#define LIST_SIZE_F  1500000

    char  t6_node_mark[TREE_SIZE_F];
    int   t6_node_val[TREE_SIZE_F][4];
    int   t6_node_val2[TREE_SIZE_F];

    int   t6_node_ptr[TREE_SIZE_F][3];
    int   t6_root_ptr;
    int   t6_buff_ptr;
    ...
    int   t6_list_stack[LIST_SIZE_F];
    char  t6_list_stack_type[LIST_SIZE_F];
    int   t6_list_ptr;

    //int   t6_out_buff[TREE_SIZE_F][5];
    ...
    int   t6_search_node(int pn1,int pn2,int pn3,int pn4);
    int   t6_insert_node(int pn1,int pn2,int pn3,int pn4);
    int   t6_dsp_tree2(void);
    int   t6_after_list(void);
    int   t6_out_list(char *pstr,int ,int);
    int   t6_dsp_list(void);
    int   t6_save_list(char *fn);

    int t6_istrcmp(int i,int pn1,int pn2,int pn3,int pn4/*,int pn5*/);


int t6_init_tree2(void)
{
  int i,j;
  for (i=0;i<TREE_SIZE_F;i++)
  {
    t6_node_mark[i]=(-1);
  }
  t6_root_ptr=(-1);
...
int t6_new_node(void)
{
  ...
  if ((t6_buff_ptr<TREE_SIZE_F)&&(t6_node_mark[t6_buff_ptr]<0))
  {
    t6_node_mark[t6_buff_ptr]=0;
    i=t6_buff_ptr;
    t6_buff_ptr++;
  }
...
int t6_insert_node(int pn1,int pn2,int pn3,int pn4/*,int pn5*/)
{
  ...
  else
  {
    if (t6_find_ptr<0)
    {
      ...
      if (j<0)
      {
        MessageBox(0,"In tree6,error at insert_node() when call new_node()","message",MB_OK);
        return(1);
      }
      else
      {
...
/*
	if (t6_node_val[i][4]>pn5) return(1);
	else
	{
		if (t6_node_val[i][4]<pn5) return(-1);
		else
  ...
  while (t6_list_ptr>0)
  {
    ...
    if (t6_list_stack_type[j]==1)
    {
      ...
      if (t6_node_ptr[k][1]>=0)
      {
        ...
        if (t6_list_ptr>=LIST_SIZE_F)
        {
          MessageBox(0,"In tree6,error in after_list(),LIST_SIZE_F too small.","message",MB_OK);
          continue;
        }
    ...
    else
    {
      ...
      //t6_out_list(t6_node_val[k],t6_node_val2[k],k);
...
int t6_out_list(char *pstr,int pn1,int ptr)
{
  int i,j;
/*
  if ((int)strlen(pstr)>=300) return(0);
  
  strcpy(t6_out_buff[t6_out_ptr],pstr);
  t6_out_buff2[t6_out_ptr]=pn1;


  for (i=0;i<5;i++)
	for (j=0;j<3;j++)
		t6_out_buff3[t6_out_ptr][i][j]=t6_node_val3[ptr][i][j];

  t6_out_buff3_ptr[t6_out_ptr]=t6_node_val3_ptr[ptr];
...
int t6_save_list(char *fn)
{
  ...
  for (i=0;i<t6_out_ptr;i++)
  {
    fputs(t6_out_buff[i],fp);
    //fputs(",",fp);

    //sprintf(m05_str1,"%lld",t6_out_buff2[i]);
    //fputs(m05_str1,fp);
