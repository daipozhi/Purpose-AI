...
int /*tree2::*/t1_new_node(void)
{
  ...
  if ((t1_buff_pp<TREE2_SIZE_A)&&(t1_node_mark[t1_buff_pp]<0))
  {
  ...
  while (t1_list_pp>0)
  {
    ...
    if (t1_list_stack_type[j]==1)
    {
      ...
      if (t1_node_pp[k][1]>=0)
      {
        ...
        if (t1_list_pp>=LIST_SIZE_A)
        {
          MessageBox(0,"In tree2,error in after_list(),LIST_SIZE_A too small.","message",MB_OK);
          continue;
        }
      }
