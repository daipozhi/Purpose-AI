...
char     dir_buf2[DIR_NUM][FLN_LEN];
char     dir_buf2_cs[DIR_NUM][4]; 
int      f2_eptr2;
...
int f2_loaddir2(void)
{
	FILE *fp1;
	//char s1[5000];
	int  i,j,k;

	fp1=fopen("app-charset-recgn.2.txt","r");
	if (fp1==NULL)
	{
		MessageBox(0,"app-charset-recgn.2.txt","message open file error ",MB_OK);
		return(1);
	}
	...
	while (!feof(fp1))
	{
		...
		fgets(m301_s1,500,fp1);

		string_trim_nos(m301_s1);
		
		j=(int)strlen(m301_s1);

		dir_buf2_cs[i][0]=m301_s1[1];
		dir_buf2_cs[i][1]=m301_s1[2];
		
		dir_buf2_cs[i][2]=m301_s1[21];
		dir_buf2_cs[i][3]=m301_s1[22];
		
		for (k=31;k<j;k++)
		{
		    if (k-31+0>=FLN_LEN) continue;
		    if (k-31+1>=FLN_LEN) continue;
		    if (k>=5000) continue;
		    
		    dir_buf2[i][k-31+0]=m301_s1[k];
		    dir_buf2[i][k-31+1]=0;
		}
